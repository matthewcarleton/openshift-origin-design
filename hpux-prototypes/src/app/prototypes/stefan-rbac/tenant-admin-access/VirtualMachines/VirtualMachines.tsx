...
const VirtualMachines: React.FunctionComponent<VirtualMachinesProps> = ({ hubClusterOnly = false, showProjectsOnly = false }) => {
  ...
  // Define tenant admin cluster sets filter
  ...
  const dbClusterSets = React.useMemo(() => 
    getAllClusterSets().filter(cs => TENANT_ADMIN_CLUSTER_SETS.includes(cs.name)), 
    []
  );
    ...
    return vmsFromDB
      .filter(vm => {
        ...
        const matchesSearch = !searchValue || 
          ...
          (vm.ipAddress && vm.ipAddress.toLowerCase().includes(searchLower)) ||
          (vm.clusterId && vm.clusterId.toLowerCase().includes(searchLower)) ||
          (vm.namespaceId && vm.namespaceId.toLowerCase().includes(searchLower));
      ...
      .map((vm, index) => {
        ...
        const clusterSet = dbClusterSets && vm.clusterId
          ? dbClusterSets.find(cs => 
  ...
  const breadcrumbHierarchy = React.useMemo(() => {
        ...
        if (allSameCluster) {
          ...
          if (commonCluster) {
            ...
            if (!commonCluster.clusterSetId || commonCluster.clusterSetId === '') {
              ...
              commonClusterSet = dbClusterSets.find(cs => cs.id === commonCluster!.clusterSetId) || null;
            }
          }
        }
      }
    ...
    if (type === 'clusterset') {
      clusterSet = dbClusterSets.find(cs => cs.id === id);
      ...
      if (cluster) {
        ...
        if (!cluster.clusterSetId || cluster.clusterSetId === '') {
          ...
          clusterSet = dbClusterSets.find(cs => cs.id === cluster.clusterSetId);
        }
      }
            ...
            <FlexItem flex={{ default: 'flex_1' }}>
              <div style={{ position: 'relative' }}>
                ...
                {isSearchMenuOpen && (() => {
                  ...
                  // Search cluster sets
                  const matchingClusterSets = dbClusterSets
