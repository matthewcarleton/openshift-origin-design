...
const GroupDetail: React.FunctionComponent = () => {
  ...
  const prepareInitialDataForEdit = (assignment: RoleAssignment) => {
    ...
    const allClusterSets = getAllClusterSets().filter(cs => TENANT_ADMIN_CLUSTER_SETS.includes(cs.name));
    const filteredClusterSets = allClusterSets;
    ...
    const mockClusterSets = filteredClusterSets.map((clusterSet, index) => ({
      id: index + 1,
  ...
  const handleWizardComplete = (wizardData: any) => {
    ...
    const allRoles = getAllRoles().filter(role => role.category !== 'open-cluster-management');
