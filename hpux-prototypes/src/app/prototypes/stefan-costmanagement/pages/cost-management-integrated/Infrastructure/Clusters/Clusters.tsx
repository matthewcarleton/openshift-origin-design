...
// Get cost data from dataService
...
const mockClusters = dbClusters.map((cluster, index) => {
  // Find matching cluster in cost management data by name or ID
  const costCluster = costManagementClusters.find(
    cc => cc.name === cluster.name || cc.id === cluster.id || cc.displayName === cluster.name
  );
  
  // Calculate cost and percentage
  const clusterCost = costCluster?.cost || 0;
  ...
  return {
    ...
    cost: dataService.formatCurrency(clusterCost),
