...
const Overview: React.FunctionComponent = () => {
  ...
  // Get all VMs from the database - filter for hub cluster only (single cluster)
  const allVMs = useMemo(() => {
    const vms = getAllVirtualMachines();
    
    // For Core Platforms perspective, only show VMs from the hub cluster
  ...
  const [resourceGrouping, setResourceGrouping] = useState<'templates' | 'projects'>('templates');
          ...
          <Card isFullHeight>
            <CardBody>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                ...
                <Dropdown
                  ...
                  toggle={(toggleRef: React.Ref<MenuToggleElement>) => (
                    ...
                    >
                      Show VirtualMachine per {resourceGrouping === 'templates' ? 'OS' : 'Projects'}
                    </MenuToggle>
                  )}
                >
                  <DropdownList>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '200px' }}>
                {/* Donut chart */}
                {(() => {
                  const dataSource = resourceGrouping === 'templates' ? metrics.vmsByTemplate : metrics.vmsByProject;
