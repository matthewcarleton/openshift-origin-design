...
const Settings: React.FunctionComponent = () => {
  ...
  const [aaqEnabled, setAaqEnabled] = useState(true);
                      ...
                      <div style={{ padding: '0 24px 24px 56px' }}>
                        ...
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <Content>Enable Application Aware Quota (AAQ)</Content>
                              <Tooltip
                                content="AAQ manages resource quotas specifically for VMs. Unlike Kubernetes ResourceQuotas, AAQ is virtualization-aware and prevents issues such as failed live migrations due to quota limits."
                                position="top"
                              >
                                <InfoCircleIcon style={{ color: 'var(--pf-t--global--icon--color--subtle)', fontSize: '14px', cursor: 'pointer' }} />
                              </Tooltip>
                              <Label color="blue" isCompact>New</Label>
                            </div>
                            <Switch
                              id="aaq-switch"
                              aria-label="Enable Application Aware Quota (AAQ)"
                              isChecked={aaqEnabled}
                              onChange={(_event, checked) => setAaqEnabled(checked)}
                            />
                          </div>
                          <Link 
                            to="/core/virtualization/quotas" 
                            style={{ 
                              color: 'var(--pf-t--global--color--brand--default)', 
                              fontSize: '14px', 
                              paddingLeft: 0, 
