...
const TopConsumers: React.FunctionComponent = () => {
  // Get VMs from hub cluster only
  const hubClusterVMs = useMemo(() => {
    const vms = getAllVirtualMachines();
    return vms.filter(vm => vm.clusterId === 'cluster-hub');
  }, []);
  ...
  // Calculate consumer data from hub cluster VMs only
  const cpuConsumers = useMemo(() => {
    if (hubClusterVMs.length === 0) return [];
    const sortedByUsage = [...hubClusterVMs]
      .sort((a, b) => b.cpu - a.cpu)
      .slice(0, parseInt(selectedTopCount.match(/\d+/)?.[0] || '5'));
    const maxUsage = sortedByUsage[0]?.cpu || 1;
    return sortedByUsage.map(vm => ({
      name: vm.name,
      usage: vm.cpu,
      maxUsage
    }));
  }, [hubClusterVMs, selectedTopCount]);

  const memoryConsumers = useMemo(() => {
    if (hubClusterVMs.length === 0) return [];
    const sortedByUsage = [...hubClusterVMs]
      .map(vm => ({
        name: vm.name,
        usage: parseFloat(vm.memory.split(' ')[0]) // Extract GiB value
      }))
      .sort((a, b) => b.usage - a.usage)
      .slice(0, parseInt(selectedTopCount.match(/\d+/)?.[0] || '5'));
    const maxUsage = sortedByUsage[0]?.usage || 1;
    return sortedByUsage.map(vm => ({
      ...vm,
      maxUsage
    }));
  ...
  // Storage throughput - mock data with limited values based on hub cluster VMs
  const throughputConsumers = useMemo(() => {
    if (hubClusterVMs.length === 0) return [];
    const topVMs = hubClusterVMs.slice(0, Math.min(2, hubClusterVMs.length));
    return topVMs.map((vm, index) => ({
      name: vm.name,
      usage: index === 0 ? 239.8 : 0.861,
      maxUsage: 239.8
    }));
  }, [hubClusterVMs]);

  // Storage IOPS - mock data with limited values based on hub cluster VMs
  const iopsConsumers = useMemo(() => {
    if (hubClusterVMs.length === 0) return [];
    const topVMs = hubClusterVMs.slice(0, Math.min(2, hubClusterVMs.length));
    return topVMs.map((vm, index) => ({
      name: vm.name,
      usage: index === 0 ? 0.03 : 0.00,
      maxUsage: 0.03
    }));
  }, [hubClusterVMs]);

  // Storage write latency - mock data with limited values based on hub cluster VMs
  const writeLatencyConsumers = useMemo(() => {
    if (hubClusterVMs.length === 0) return [];
    const topVMs = hubClusterVMs.slice(0, Math.min(2, hubClusterVMs.length));
    return topVMs.map((vm, index) => ({
      name: vm.name,
      usage: index === 0 ? 0.008 : 0.001,
      maxUsage: 0.008
    }));
  }, [hubClusterVMs]);
